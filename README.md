...
>Проект разработан в учебных целях с помощью DJANGO REST FRAMEWORK и не рекомендуется для коммерческого использования, характеризуется 
...
6. **Создание суперпользователя**: Создайте суперпользователя, который будет иметь доступ к административной панели
   ...
   ```
7. **Активация Redis**: Необходимо активировать **Redis** на вашем сервере для хранения данных с помощью
   следующей команды:
   ```commandline
   sudo service redis-server start
   ```
8. **Активация Celery Worker**: Необходимо активировать **Celery Worker** на вашем сервере для выполнение асинхронных
...
1. **Покупатель (г. user)**: роль, предназначенная для учеников и всех, кто ищет образовательные возможности. Покупатели могут
